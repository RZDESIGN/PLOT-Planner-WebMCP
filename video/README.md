...
After recording, a separate native WebMCP check passed on September 2, 2026: Codex's in-app browser discovered all 17 page-defined tools at `http://localhost:5173/` and successfully called `plot.get_board` and `plot.analyze_board`. This establishes native local discovery and read calls; it does not change the film's tool-handler recording provenance or establish public-origin behavior.

Publication details and copy are recorded in `youtube-upload.md`. The final domain is `https://plotplanner.xyz/`; public-origin verification is pending DNS. Future edits can be rendered locally before deciding whether to replace the public demonstration.
